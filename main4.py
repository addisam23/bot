...
Daily_bonus = 0.00  # Set daily bonus amount here!
...
def start(message):
    try:
        ...
        if user not in data.get('withd', {}):
            data['withd'][user] = 0
        if user not in data.get('id', {}):
...
def query_handler(call):
    try:
        ch = check(call.message.chat.id)  # Assuming `check` is defined somewhere.
        if call.data == 'check':
            if ch:
                data = load_users_data()
...
def trx_address(message):
    try:
        ...
        if len(message.text) == 34:
            ...
            bot.send_message(message.chat.id, f"*💹 Your TRX wallet is set to {data['wallet'][user]}*", parse_mode="Markdown")
            save_users_data(data)
...
def amo_with(message):
    try:
        ...
        bot.send_message(user_id, f"✅* Withdraw is requested to our owner automatically\n\n💹 Payment Channel :- {PAYMENT_CHANNEL}*", parse_mode="Markdown")
